...
boot_alloc(uint32_t n)
{
	...
	// which points to the end of the kernel's bss segment:		指向内核段的.bss段末尾
	// the first virtual address that the linker did *not* assign
	// to any kernel code or global variables.
	// cprintf("nextfree1 is %08x\n",nextfree);
	if (!nextfree) {
		extern char end[];		// 定义在kern/kernel.ld
		// cprintf("end is %08x\n",end);
	...
	// cprintf("nextfree2 is %08x\n",nextfree);
	// cprintf("mem is %08x\n",((uint32_t)nextfree - KERNBASE)>>12);
...
mem_init(void)
{
	...
	// 分配一个 npages 数组 'struct PageInfo' 并将其存储在 'pages' 中。
	// 内核使用这个数组来跟踪物理页面：对于每个物理页面，这个数组中都有一个对应的结构 PageInfo。 
...
page_init(void)
{
	...
	//     is free.
	//  3) Then comes the IO hole [IOPHYSMEM, EXTPHYSMEM)(0x0A0000(640kb),0x100000), which must
	//     never be allocated.
	...
	// free pages!
	// pages是一个数组，数组长度为物理页面个数，每个数组元素为一个struct,包含pp_ref和pp_link
	size_t i;
	...
	// cprintf("i is %d\n", i);
	// cprintf("free_list size is %d\n", npages-i+npages_basemem);
	for (; i < npages; i++) {
		pages[i].pp_ref = 0;
		pages[i].pp_link = page_free_list;
		page_free_list = &pages[i];
	...
	// 	if(num++ % 100 == 0)cprintf("%08x ", page_free_list);
...
page_alloc(int alloc_flags)
{
	...
	if (page_free_list) 
	{
		struct PageInfo* tem = page_free_list;
		page_free_list = page_free_list->pp_link;
		tem->pp_link = NULL;
...
page_free(struct PageInfo *pp)
{
	...
	if (pp->pp_ref > 0 || pp->pp_link != NULL) 
		panic("Page table entries point to this physical page.");
...
//
// The relevant(相关) page table page might not exist yet.
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if (!(pgdir[dir] & PTE_P)) {
		...
		pg->pp_ref++;
		pgdir[dir]  = page2pa(pg) | PTE_P | PTE_U | PTE_W;
	}
...
// 将虚拟地址空间的 [va, va+size) 映射到以 pgdir 为根的页表中的物理 [pa, pa+size)。
// Size 是 PGSIZE 的倍数，va 和 pa 都是页面对齐的。
// 对条目使用权限位 perm|PTE_P。
//
...
// frequently leads to subtle(微妙的) bugs; there's an elegant way to handle
...
{
	// Fill this function in
	// pte_store实际是传进来的*pte_t的地址，如果pte存在，则将该pte传给pte_store，方便后续page_remove，避免两次调用pgdir_walk()函数
