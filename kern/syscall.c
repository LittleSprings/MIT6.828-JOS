...
sys_env_set_trapframe(envid_t envid, struct Trapframe *tf)
{
	...
	struct Env *e;
	int r;
	if (r = envid2env(envid, &e, 1) < 0 || (e->env_tf.tf_cs & 3) != 3)
		return r;
	if ((r = user_mem_check(e, tf, sizeof(struct Trapframe), PTE_U)) < 0) {
		return r;
	}
	e->env_tf = *tf;
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	switch (syscallno) {
		...
		case SYS_exofork:
			// cprintf("kernel syscall exofork start\n");
			int ret = sys_exofork();
			// cprintf("sys_exofork() return is %x\n", ret);
			return ret;
		...
		case SYS_packet_try_send:
			return sys_packet_try_send((void *) a1, (uint32_t) a2);
		case SYS_packet_receive:
			return sys_packet_receive((void *) a1, (uint32_t) a2);
        default:
            return -E_INVAL;
	}
